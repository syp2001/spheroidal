            ...
            for ell in ells:
                ...
                for m in m:
                    with self.subTest(s=s,ell=ell,m=m):
                        self.assertEqual(ell*(ell+1)-s*(s+1),spheroidal.eigenvalue(s,ell,m,0))
        ...
        for s in spins:
            ...
            for i,ell in enumerate(ells):
                ...
                for j,m in enumerate(m):
                    with self.subTest(s=s,ell=ell,m=m):
                        self.assertAlmostEqual(data[i,j],spheroidal.eigenvalue(s,ell,m,g,method="spectral"))
    ...
    def test_leaver_eigenvalue(self):
        ...
        for s in spins:
            ...
            for i,ell in enumerate(ells):
                ...
                for j,m in enumerate(m):
                    with self.subTest(s=s,ell=ell,m=m):
                        self.assertAlmostEqual(data[i,j],spheroidal.eigenvalue(s,ell,m,g,method="leaver"))