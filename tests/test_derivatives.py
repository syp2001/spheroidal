...
class TestHarmonics(unittest.TestCase):
    ...
    def test_spherical_expansion_second_derivative(self):
        ...
        for s in spins:
            ...
            for ell in ells:
                # load data and reshape so that the s=ell=0 case isn't a 1D array
                data = np.loadtxt(DATA_DIR / str("s"+str(s)+"/ell"+str(ell)+"_deriv2.txt"),dtype=np.cdouble).reshape((len(theta),int(2*ell+1)))
                # generate all possible m values
                m = np.arange(-ell,ell+1,1)
                for i,m in enumerate(m):
                    Sslm = spheroidal.harmonic_deriv(s,ell,m,g,n_theta=2,method="spectral")
                    for j,th in enumerate(theta):
                        with self.subTest(s=s,ell=ell,m=m,theta=th):
                            if (th != 0) and (th != pi): self.assertAlmostEqual(abs(data[j,i]),abs(Sslm(th,0)),places=1)
    ...
    def test_leaver_second_derivative(self):
        ...
        for s in spins:
            ...
            for ell in ells:
                ...
                m = np.arange(-ell,ell+1,1)
                for i,m in enumerate(m):
                    Sslm = spheroidal.harmonic_deriv(s,ell,m,g,n_theta=2,method="leaver")
                    for j,th in enumerate(theta):
                        with self.subTest(s=s,ell=ell,m=m,theta=th):
                            if (th != 0) and (th != pi): self.assertAlmostEqual(abs(data[j,i]),abs(Sslm(th,0)),places=1)