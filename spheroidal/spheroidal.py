    ...
    * "numerical leaver": numerically differentiates Leaver's method
    ...
    if wrt == "theta":
        ...
        if method == "numerical leaver":
            ...
            return lambda theta,phi: (-S(theta+2*dx,phi)+8*S(theta+dx,phi)-8*S(theta-dx,phi)+S(theta-2*dx,phi))/(12*dx)
    ...
    if wrt == "phi":
        ...
        if method == "numerical leaver":
            S = harmonic_leaver(s,ell,m,g,num_terms)
            return lambda theta,phi: (-S(theta,phi+2*dx)+8*S(theta,phi+dx)-8*S(theta,phi-dx)+S(theta,phi-2*dx))/(12*dx)
        if method == "numerical spectral":
            S = harmonic_spectral(s,ell,m,g,num_terms,n_max)
            return lambda theta,phi: (-S(theta,phi+2*dx)+8*S(theta,phi+dx)-8*S(theta,phi-dx)+S(theta,phi-2*dx))/(12*dx)
        raise ValueError("Invalid method: {}".format(method))
...
def harmonic_spectral_deriv(s,ell,m,g,num_terms,n_max=100):
    ...
    def dS(theta,phi):
        spherical_harmonics = np.array([sphericalY_deriv(s,l,m)(theta,phi) for l in np.arange(l_min,l_min+len(coefficients))])
        return spherical_harmonics.dot(coefficients)
...
def harmonic_leaver_deriv(s,ell,m,g,num_terms=None):
    ...
    # Slm = exp(g*u) * sqrt(x^(2*k1)*(2-x)^(2*k2)) * sum_0^inf a_n x^n where x = 1+u = 1+cos(theta)
    radicand =  Polynomial([0,1])**int(2*k1)*Polynomial([2,-1])**int(2*k2)
    radicand_deriv = radicand.deriv()
    series = Polynomial(a)
    ...
    def dS(theta,phi):
        ...
        sqrt_term = sqrt(abs(radicand(x)))
        # differentiate series using product/chain rule
        return -np.sin(theta)*(g*np.exp(g*u)*sqrt_term*series(1+u)+
                               np.exp(g*u)*sqrt_term*series_deriv(1+u)+
                               1/2*np.exp(g*u)*radicand_deriv(x)/sqrt_term*series(1+u))*np.exp(1j*m*phi)
