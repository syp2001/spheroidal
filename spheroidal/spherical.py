...
def sphericalY(s,l,m):
    ...
    prefactor = (-1.)**(l+m-s+0j)
    prefactor *= sqrt(factorial(l+m)*factorial(l-m)*(2*l+1)/(4*pi*factorial(l+s)*factorial(l-s)))
    
    def Y(theta,phi):
        #if theta == 0: theta = 1e-14
        alternating_sum = 0
        for r in range(int(max(m-s,0)),int(min(l-s,l+m)+1)):
            alternating_sum = alternating_sum + (-1)**r*binom(l-s,r)*binom(l+s,r+s-m)*sin(theta/2)**(2*l-2*r-s+m)*cos(theta/2)**(2*r+s-m)
...
def sphericalY_deriv(s,l,m):
    ...
    """
    # https://en.wikipedia.org/wiki/Spin-weighted_spherical_harmonics
    prefactor = (-1.)**(l+m-s+0j)*sqrt(factorial(l+m)*factorial(l-m)*(2*l+1)/(4*pi*factorial(l+s)*factorial(l-s)))
    ...
    def dY(theta,phi):
        ...
        for r in range(int(max(m-s,0)),int(min(l-s,l+m)+1)):
            alternating_sum_deriv = alternating_sum_deriv + \
                  (-1)**r*binom(l-s,r)*binom(l+s,r+s-m)*sin(theta/2)**(2*l-2*r-s+m-1)*cos(theta/2)**(2*r+s-m-1)*(-2*r-s+m)/2
            alternating_sum = alternating_sum + (-1)**r*binom(l-s,r)*binom(l+s,r+s-m)*sin(theta/2)**(2*l-2*r-s+m-1)*cos(theta/2)**(2*r+s-m+1)
...
def c1(s,m,j,l):
    ...
    :rtype: double
    """
    return sqrt((2*l+1)/(2*j+1))*float(clebsch_gordan(l,m,1,0,j,m).doit())*float(clebsch_gordan(l,-s,1,0,j,-s).doit())
...
def c2(s,m,j,l):
    ...
    :rtype: double
    """
    return (1/3 if j==l else 0) + 2/3*sqrt((2*l+1)/(2*j+1))*float(clebsch_gordan(l,m,2,0,j,m).doit())*float(clebsch_gordan(l,-s,2,0,j,-s).doit())
...
def spectral_matrix_bands(s,m,g,num_terms,offset=0):
    ...
    return [[g**2*c2(s,m,l,l)-2*g*s*c1(s,m,l,l)-l*(l+1) for l in np.arange(offset+l_min,offset+l_min+num_terms,1)],
            [g**2*c2(s,m,l+1,l)-2*g*s*c1(s,m,l+1,l) for l in np.arange(offset+l_min,offset+l_min+num_terms,1)],
            [g**2*c2(s,m,l+2,l) for l in np.arange(offset+l_min,offset+l_min+num_terms,1)]]
